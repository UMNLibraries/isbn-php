...
class Data
{
  ...
  {
    if (!isset(self::$groupData)) {
      self::$groupData = self::generateGroupData();
    }
    return self::$groupData;
  }

  protected static $maxGroupCodeLength;
  public static function maxGroupCodeLength()
  {
    if (!isset(self::$maxGroupCodeLength)) {
      self::$maxGroupCodeLength = self::generateMaxGroupCodeLength();
    }
    return self::$maxGroupCodeLength;
  }

  public static function groupCodeExists($groupCode)
  {
    $groupData = self::groupData();
    return array_key_exists($groupCode, $groupData) ? true : false;
  }

  public static function getPublisherRanges($groupCode)
  {
    $groupData = self::groupData();
    $groupCodeData = $groupData[$groupCode];
    ...
    foreach ($groupCodeData as $description => $publisherRanges) {
      break;
    }
    return $publisherRanges;
  }

  protected static function generateGroupData()
  {
    return array
      (
  ...
  public static function generateMaxGroupCodeLength()
  {
    $groupData = self::groupData();
    krsort($groupData, SORT_NUMERIC);

    // This seems like a clunky way to find the last key, but can't
    // find a better way yet in PHP:
    foreach ($groupData as $groupCode => $values) {
      $longestKey = $groupCode;
      break;
    }
    return strlen($longestKey);
  }
