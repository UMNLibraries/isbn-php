...
abstract class ISBN extends Moose
{
...
*/
    abstract static function max_length();
    abstract static function parse_prefix( $isbn );
    abstract function hyphen_positions();

    public static function max_publisher_code_length()
    {
        $args = func_get_args();
        $params = $args[0];
        $isbn = $params['isbn'];
...
*/
    ...
    protected function properties()
    {
        ...
        self::has('input_isbn', array('is' => 'protected', 'required' => 0)); 
        self::has('prefix', array('is' => 'protected', 'required' => 0,)); 
        self::has('group_code', array('is' => 'protected', 'required' => 0,)); 
        self::has('publisher_code', array('is' => 'protected', 'required' => 0,)); 
        self::has('article_code', array('is' => 'protected', 'required' => 0,)); 
        self::has('checksum', array('is' => 'protected', 'required' => 0,)); 
    }

    protected function BUILD( $args )
    {
        // We don't really need the args in this case...
        //$input_isbn = $args[0];
        
        $isbn = $this->isbn();
        if ($this->input_isbn() == NULL) {
            $this->set_input_isbn( $isbn );
        }
        ...
        $prefix = call_user_func_array(
            array($class, 'parse_prefix'),
            array( $isbn )
        );
        $this->set_prefix( $prefix );

        $group_code = self::parse_group_code(array(
            'isbn' => $isbn,
            'prefix' => $prefix,
        ));
        $this->set_group_code( $group_code );

        $publisher_code = self::parse_publisher_code(array(
            'isbn' => $isbn,
            'prefix' => $prefix,
            'group_code' => $group_code,
        ));
        $this->set_publisher_code( $publisher_code );

        $article_code = self::parse_article_code(array(
            'isbn' => $isbn,
            'prefix' => $prefix,
            'group_code' => $group_code,
            'publisher_code' => $publisher_code,
        ));
        $this->set_article_code( $article_code );

        $checksum = self::parse_checksum( $isbn );
...
*/
    protected function checksum_pos()
    {
...
*/
    public function as_string()
    {
        $args = func_get_args();
        $hyphen_positions = 
            (array_key_exists(0, $args) && is_array($args[0])) ? $args[0] : $this->hyphen_positions();

        $isbn = $this->isbn();
        
        rsort($hyphen_positions, SORT_NUMERIC);
...
*/
    ...
    public static function normalize( $input_isbn )
    {
        // We want uppercase X's:
        $input_isbn = strtoupper( $input_isbn );

	// Get rid of everything except decimal digits and X:
        $input_isbn = preg_replace('/[^0-9X]/', '', $input_isbn);
        
        if (preg_match('/^(?:\d\d\d)?\d{9}[0-9X]$/', $input_isbn)) {
            return $input_isbn;
        }
        throw new Exception("Input ISBN '$input_isbn' does not appear to be an ISBN");
    }
...
*/
    public static function parse_group_code( $isbn )
    {
        $args = func_get_args();
        $params = $args[0];
        $isbn = $params['isbn'];
        ...
        foreach (range(1, ISBN_Data::max_group_code_length()) as $count) {
            $group_code = substr($isbn, $prefix_length, $count);
            if (ISBN_Data::group_code_exists( $group_code )) {
                return $group_code;
            }
        }
        throw new Exception("Cannot find a valid group code in ISBN '$isbn'");
    }
...
*/
    public static function parse_publisher_code()
    {
        $args = func_get_args();
        $params = $args[0];
        $isbn = $params['isbn'];
        ...
        $max_publisher_code_length = 
            self::max_publisher_code_length( $params );

        $longest = substr(
            $isbn,
            strlen($prefix) + strlen($group_code),
            $max_publisher_code_length
        );

        $publisher_ranges = 
            ISBN_Data::get_publisher_ranges( $group_code );
        foreach ($publisher_ranges as $lower => $upper) {

            $publisher_code = substr($longest, 0, strlen($lower));
            if ($publisher_code >= $lower && $publisher_code <= $upper)
                return $publisher_code;
        }
        throw new Exception("Cannot find a valid publisher code in ISBN '$isbn'");
    }
...
*/
    public static function parse_article_code()
    {
        $args = func_get_args();
        $params = $args[0];
        $isbn = $params['isbn'];
        $prefix = $params['prefix'];
        $group_code = $params['group_code'];
        $publisher_code = $params['publisher_code'];

        $head = strlen($prefix) + strlen($group_code) + strlen($publisher_code);
...
*/
    public static function parse_checksum( $isbn )
    {
        $class = 'ISBN' . strlen( $isbn );
        preg_match('/^(\d+)([\dX])$/', $isbn, $matches);
        $isbn_minus_checksum = $matches[1];
        $checksum = $matches[2];
        $calculated_checksum = call_user_func_array(
            array($class, 'calculate_checksum'),
            array($isbn_minus_checksum)
        );
        if ($checksum != $calculated_checksum) {
            throw new Exception("ISBN '$isbn' has an invalid checksum.");
        }
        return $checksum;
    }
